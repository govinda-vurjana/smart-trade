...
# Process image to extract candlestick data
# Process image to extract candlestick data
def process_image(image):
    ...
    for i, (x, y, w, h) in enumerate(bounding_boxes):
        ...
        # Check dominant color in the candlestick body
        hue_values = roi[..., 0].flatten()  # Get hue values
        ...
        if green_ratio > red_ratio:
            open_price, close_price = high, low  # Green (Bullish)
            color = "Green"
        else:
            open_price, close_price = low, high  # Red (Bearish)
            color = "Red"

        all_candles_data.append([i, open_price, high, close_price, low, x, y, w, h, color])

    print("✅ Candlestick data corrected with accurate color classification.")
